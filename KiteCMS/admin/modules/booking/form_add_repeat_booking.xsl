...
function funcsubmit(){
	...
	if(!regex.test(document.booking.enddate.value))
		if(document.booking.returning[0].checked==false){
...
<input type="hidden" name="action" value="doaddrepeat"/>
...
<tr>
	...
	<td valign="top"><xsl:value-of select="util:localText('not')"/>:<input type="radio" name="returning" value="no" checked=""/>&#160;<xsl:value-of select="util:localText('dayly')"/>:<input type="radio" name="returning" value="day"/>&#160;<xsl:value-of select="util:localText('weekly')"/>:<input type="radio" name="returning" value="week"/></td>
</tr>
<tr>
	<td valign="top" align="right">Evt. slutdato (dd-mm-åååå):</td>
	<td>
		<input type="text" name="enddate" value="" size="15" maxlenght="50" class="alminput"/>
	</td>
</tr>
<tr>
	<td valign="top" align="right">Start tidspunkt:</td>
	<td>
...
<tr>
	<td valign="top" align="right">Antal lektioner:</td>
	<td>
...
<tr>
	<td></td>
	<td>
	<input type="submit" class="almknap" value="{util:localText('add')}" onclick="return funcsubmit();"/>&#160;
