...
<xsl:template match="/">
	...
	<input type="hidden" name="action" value="doaddcalendar"/>
	<xsl:for-each select="/website/calendar/field">
		...
		<xsl:choose>
			...
			<xsl:when test="@type='text'">
				<input id="field{$name}" name="field{$name}" size="{@size}" class="alminput" value=""/>
			</xsl:when>
			<xsl:when test="@type='largetext'">
				<textarea id="shopitem{$name}" name="field{$name}" class="alminput" cols="{@cols}" rows="{@rows}"></textarea>
			</xsl:when>
