...
<xsl:template match="/">
	...
	<form name="editshopitem" id="editshopitem" onsubmit="return funcSubmit(this);" method="post" action="default.aspx">
	<input type="hidden" name="action" value="doeditshopitem"/>
	<input type="hidden" name="pageid" value="{//selectedpage}"/>
	<input type="hidden" name="shopitem" value="{$selectedshopitem}"/>
	<xsl:for-each select="/shop/productstore/field">
		<xsl:variable name="name"><xsl:value-of select="@id"/></xsl:variable>
		<tr><td valign="top"><label for="shopitem{$name}"><xsl:value-of select="@label" disable-output-escaping="yes"/></label></td><td>
		<xsl:choose>
				...
				<select id="shopitem{$name}" name="shopitem{$name}" class="alminput">
					...
					<xsl:for-each select="//category">
						<xsl:element name="option">
							<xsl:if test="//shopitem[@id=$selectedshopitem]/item[@id=$name]=@id">
	...
	<tr><td></td><td><input value="{util:localText('save')}" type="submit" class="almknap"/>&#160;
		<xsl:element name="input">
		...
		<xsl:attribute name="onclick">if(confirm('<xsl:value-of select="util:localText('oktodelete')"/>')) {document.location='default.aspx?pageid=<xsl:value-of select="//selectedpage"/>&amp;shopitem=<xsl:value-of select="$selectedshopitem"/>&amp;action=dodeleteshopitem';}</xsl:attribute>
		</xsl:element>&#160;
<input type="button" value="{util:localText('cancel')}" class="almknap" onclick="history.go(-1);"/></td></tr>
	</form>
	</table>
</xsl:template>
</xsl:stylesheet>