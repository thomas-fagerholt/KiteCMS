...
namespace KiteCMS.Admin
{
	...
	/// </summary>
	public class edittemplate : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			admin.userHasAccess(1402,page.Pageid);
			header.Text = Functions.localText("edittemplate");
			...
			if (action == "save")
				...
				funcFormEditTemplate();
			else
				funcEditTemplate();

		}

		private void funcDoEditTemplate()
		{
			int templateId = -1;
			string title = "";
			string publicurl = "";
            ...
            if (HttpContext.Current.Request["usercontrol"] != null)
                usercontrol = HttpContext.Current.Request["usercontrol"].ToString();
			...
			if (templateId != -1 && title != "" && publicurl != "" && xslurl != "" && adminurl != "")
			{
                ...
                template.UserControl = usercontrol;
		...
		private void funcFormEditTemplate()
		{
			int templateId = -1;
		...
		private void funcEditTemplate()
		{
			...
			content.Text += Functions.transformMenuXml(Server.MapPath("/admin/template/edit_template.xsl"),xslArg);
