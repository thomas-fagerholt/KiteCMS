...
<xsl:for-each select="//page[@id=//selectedpage]">	
	...
	<script type="text/javascript">
		var strUrl = '<xsl:value-of select="//template[@id=$moduleid]/publicurl"/>?pageid=<xsl:value-of select="//selectedpage"/>';
	</script>
...
<xsl:if test="count(//page[@id=//selectedpage])=0">
	...
	<xsl:element name="a">
	<xsl:attribute name="href"><xsl:value-of select="//template[@id=$moduleid]/publicurl"/>?pageid=<xsl:value-of select="//page/@id"/></xsl:attribute>
	<xsl:value-of select="util:localText('mainpage')"/>
	</xsl:element>

  <script type="text/javascript">
    var strUrl = '<xsl:value-of select="//template[@id=$moduleid]/publicurl"/>?pageid=<xsl:value-of select="//page/@id"/>';
	</script>
</xsl:if>
